...
class HRDAHead(BaseDecodeHead):
    ...
    def decode_hr(self, inp, bs,return_context = False):
    ...
    def forward(self, inputs,return_context = False):
        ...
        if return_context:
            lr_seg, lr_context = self.head(lr_inp,return_context)
        else :
            lr_seg = self.head(lr_inp, return_context)
        ...
        if return_context:
            return fused_seg, lr_seg, hr_seg,lr_context
    ...
    def forward_train(self,
        ...
        seg_logits = self.forward(inputs,return_context)
        if return_context:
            lr_context = seg_logits[-1]
            losses = self.losses(seg_logits[:3], gt_semantic_seg, seg_weight)
            losses['lr_context'] = lr_context
            losses['lr_logit'] = seg_logits[1]
        else:
